...
class NFA:
            ...
            if '' in self.delta[q]:
                for st in self.delta[q]['']:
                    if st not in visited:
                        visited.append(st)
                        ans.extend([k for k in self.getEClosure(
                            st, visited) if k not in ans])
        return ans
                ...
                for sigma in self.sigma:
                    ...
                    for closureState in closureStates:
                        ...
                        if closureState in self.delta and sigma in self.delta[closureState]:
                            toEpsiClosure.extend(
                                self.delta[closureState][sigma])
    ...
    def getDFA(self) -> DFA:
        ...
        localNFA = NFA(self.Q, self.sigma, self.delta,
                       self.initialState, self.F)
